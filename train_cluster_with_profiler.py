#!/usr/bin/env python3
"""
Lightweight wrapper to run the existing cluster training entrypoint while sampling memory (CPU+GPU)
and writing a CSV memory log into the training stats directory (or cwd if not set).

Usage: python train_cluster_with_profiler.py --config path/to/config.yaml --model MODEL_NAME [--device cuda]

This script reuses the `train_cluster.main()` entrypoint from `train_cluster.py` and starts a background
MemProfiler that writes a CSV file named memory_profile_<timestamp>.csv.
"""
import os
from datetime import datetime
from train_cluster import parse_args, main as train_main
...
def main():
    ...
    # Determine stats dir from config (same key used by train_cluster.py)
    stats_dir = None
    ...
    try:
        profiler.start()
        # Call the existing cluster training main() which will parse args again and run the full flow.
        train_main()
    finally:
        profiler.stop()
